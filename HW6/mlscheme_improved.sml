...
(***************NEW WORK Q 10*************)
(*
fun freeIn (VAR(l), n) = n = l
    | freeIn (SET(s, e), n) = n = s or freeIn(e,n)
    | freeIn (IFX(e1,e2,e3), n)= freeIn(e1,n) or freeIn(e2,n) or freeIn(e3,n)
    | freeIn (WHILEX(e1,e2), n) = freeIn(e1,n) or freeIn(e2,n)
    | freeIn (BEGIN(x::xs), n) = exists freeIn(x,n) xs
    | freeIn (BEGIN[], n) = true
    | freeIn (APPLY(e1, x::xs), n) = freeIn(e1,n) or
                                      foldl (fn() => or freeIn(x,n)) false xs
    | freeIn (LETX(LET,(l,exp)::xs,i), n) = (freeIn(i,n) andalso
                                not foldl (fn() => or freeIn(l,n)) false xs)
                                or foldl (fn() => or freeIn(exp,n)) false xs
    | freeIn (LETX(LETREC,[],i), n) = freeIn(i,n)
    | freeIn (LETX(LETREC,(l,exp)::xs,i), n) = (freeIn(i,n) or
                              foldl (fn() => or freeIn(exp,n))false xs)andalso
                              not foldl (fn() => or freeIn(l,n)) false xs
    | freeIn (LETX(LETREC,[],i), n) = freeIn(i,n)
    | freeIn (LETX(LETSTAR,(l,exp)::xs,i), n) = freeIn(i,n) or
                              freeIn(LET((l,exp),LETSTAR(xs,i)),n)
    | freeIn (LETX(LETSTAR,[],i), n) = freeIn(i,n)
    | freeIn (LAMBDA(x::xs, exp), n) = freeIn(exp, n) andalso not
                                foldl (fn() => or freeIn(x,n)) false xs
    | freeIn (LAMBDA([], exp), n) = freeIn(i,n);

fun improve ((xs, exp), env) = List.filter freeIn(LAMBDA(xs, exp),env) env;
*)
