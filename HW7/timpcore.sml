...
(* definitions of functions [[toy]] and [[toInt]] for \timpcore 354a *)
...
fun typeof (e, globals, functions, formals) =
  let (* function [[ty]], checks type of expression, given $\itenvs$ 347b *)
      ...
      | ty (AAT (a, i))        =
          (case (ty a, ty i) of (ARRAYTY t1, INTTY) => t1
                              | (_,INTTY) => raise TypeError
                                                    "parameter 1 not ARRATY"
                              | _ => raise TypeError "parameter 2 not INTTY")
      | ty (APUT (a, i, e))    =
          let val tau1 = ty a
              val tau2 = ty i
              val tau3 = ty e
          in if eqType(tau2,INTTY) andalso eqType(tau1, tau2) then
                 tau1
             else
                 raise TypeError "invalid input paramerters"
          end
      | ty (AMAKE (len, init)) =
          ...
          in
              if eqType (tau_len, INTTY) then
                  ...
                  raise TypeError "len not int"
          end
      | ty (ASIZE a)           =
          (case ty a of (ARRAYTY t1) => INTTY
                     | _         => raise TypeError "parameter must be ARRAYTY")
