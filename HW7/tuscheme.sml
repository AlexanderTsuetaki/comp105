...
(*****************************************************************)
(*                                                               *)
(*   \FOOTNOTESIZE SHARED: NAMES, ENVIRONMENTS, STRINGS,
ERRORS, PRINTING, INTERACTION, STREAMS, \&\ INITIALIZATION *)
(*                                                               *)
(*****************************************************************)

(* \footnotesize shared: names, environments, strings, errors, printing,
interaction, streams, \&\ initialization S237a *)
(* for working with curried functions: [[id]], [[fst]], [[snd]],
[[pair]], [[curry]], and [[curry3]] S263d *)
fun id x = x
fun fst (x, y) = x
fun snd (x, y) = y
fun pair x y = (x, y)
...
(* use [[varenv]] to substitute in [[tau]]; don't capture or substitute for any
                                                              [[alphas]] 384c *)
...
(* substitute [[varenv]] in [[FORALL (alphas, tau)]] (works only if there is no
                                                                capture) 384b *)
...
(* Boolean saying if \monobox{FORALL (alphas, tau)} $\equiv$ \monobox{FORALL
                                                        (alphas', tau')} 379b *)
      let fun ok a  =
...
exception TypeError of string
fun check_type_lists ([], []) = true
     |check_type_lists ([], type2::types2) =
              raise TypeError "incompatible lengths"
     |check_type_lists (type1::types1, []) =
              raise TypeError "incompatible lengths"
     |check_type_lists (type1::types1, type2::types2)=
         if eqType(type1, type2) then
             check_type_lists (types1, types2)
         else
             raise TypeError "incompatible types"

fun literal (v) =
  let
      fun ty (NUM int) = inttype
          | ty (BOOLV bool) = booltype
          | ty (SYM sm) = symtype
          | ty (PAIR (v, NIL)) = listtype (ty v)
          | ty (PAIR (v1, v2)) =
              let val tv1 = ty v1
                  val tv2 = ty v2
              in if eqType(listtype tv1, tv2) then
                  listtype tv1
                 else raise TypeError "improper types"
              end
          | ty (NIL) = FORALL (["'a"], listtype tvA)
          | ty _ = raise TypeError "not a literal"
  in
      ty v
  end

fun typeof (e, kind_env, type_env) =
    let
        fun ty (LITERAL v) = literal v

          | ty (VAR name) = find (name, type_env)

          | ty (SET (name, exp)) =
              let val n = ty (VAR name)
                  val e = ty exp
              in if eqType(n, e) then
                    e
                 else
                    raise TypeError ("In Set, var has type " ^
                                    typeString n ^ " but exp has type " ^
                                    typeString e)
              end
          | ty (IFX (e1, e2, e3)) =
              let val tau1 = ty e1
          ...
          | ty (WHILEX (cond, body)) =
              let val t_con = ty cond
                  val t_body = ty body
              in if eqType(t_con, booltype) then
                    unittype
                 else raise TypeError "Conditional is not a boolean"
              end
          | ty (BEGIN (exp::exps)) =
                 (case exps of (ex::[]) => ty ex
                               | _ => ty(BEGIN exps))
          | ty (APPLY (exp1, exps)) =
              (case ty exp1 of (FUNTY (type_list, result)) =>
                  let
                      val type_list2 = map ty exps
                  in
                      if check_type_lists (type_list, type_list2) then
                          result
                      else
                          raise TypeError "inputs not of correct type"
                  end
                    | _ => raise TypeError "first parameter must be function")
          | ty (LETX (let_kind, nexps, exp)) =
              (case let_kind of LET =>
               let
                  fun bind_new_env (kind_env, type_env, []) = type_env
                     | bind_new_env (kind_env, type_env, (n,e)::nexps) =
                                     bind (n, typeof (e, kind_env, type_env),
                                      bind_new_env (kind_env, type_env, nexps))
                   val new_env = bind_new_env (kind_env, type_env, nexps)
               in
                   typeof(exp, kind_env, new_env)
               end
                            | LETSTAR =>
               let
                  fun rec_LETSTAR ([], exp) = typeof(exp, kind_env, type_env)
                      | rec_LETSTAR ((n,e)::nexps, exp) =
                      typeof (LETX (LET, [(n,e)], LETX(LETSTAR, nexps, exp)),
                                                             kind_env, type_env)
               in
                   rec_LETSTAR(nexps, exp)
               end
               )
          | ty (LETRECX (exps, exp)) =
              let
                  fun bind_new_env ([]) = type_env
                     | bind_new_env ( ((x,t), e as LAMBDA _ )::exps ) =
                         if (eqKind (kindof (t, kind_env), TYPE))
                                                    andalso eqType(ty e, t) then
                                     bind (x, t, bind_new_env exps)
                         else
                              raise TypeError "type of e not type of t"
                    | bind_new_env (_) =
                                       raise TypeError "type of e not type of t"

              in
                  typeof (exp, kind_env, bind_new_env exps)
              end

          | ty (LAMBDA (parameters, lambda_exp))=
              let
                  fun bind_new_env (kind_env, type_env, []) = type_env
                     | bind_new_env (kind_env, type_env, (n,t)::nexps) =
                                     bind (n, t,
                                      bind_new_env (kind_env, type_env, nexps))
                 fun get_types ([], type_list) = type_list
                     | get_types ((n, tx)::xs, type_list) =
                         if eqKind(kindof (tx, kind_env), TYPE) then
                             get_types (xs, type_list @ [tx])
                         else
                            raise TypeError "invalid kind not * in lambda"
              in
                  FUNTY (get_types(parameters, []),
                              typeof(lambda_exp, kind_env,
                                bind_new_env(kind_env, type_env, parameters)))
              end
          | ty (TYLAMBDA (names, exp)) =
              let
                  fun bind_new_env ([]) = kind_env
                     | bind_new_env (a::nexps) =
                               bind (a, TYPE, bind_new_env nexps)
                  val tau = typeof (exp, bind_new_env names, type_env)
              in FORALL(names, tau)
              end
          | ty (TYAPPLY (exp, tyexs)) = instantiate (ty exp, tyexs, kind_env)

          | ty _ = raise TypeError "no pattern matching"
    in
        ty e
    end
...
fun typdef (d, kind_env, type_env)  =
    let
        fun ty (VAL (name, exp)) =
                if not (isbound (name, type_env)) then
                  let val tau  = typeof (exp, kind_env, type_env)
                  in  (bind (name, tau, type_env), typeString tau)
                  end
                else
                  let val tau' = find (name, type_env)
                      val tau  = typeof (exp, kind_env, type_env)
                  in  if eqType (tau, tau') then
                        (type_env, typeString tau)
                      else
                     (* raise [[TypeError]] with message about redefinition 351a
                                                                              *)
                        raise TypeError ("Global variable " ^ name ^ " of type "
                          ^ typeString tau' ^ " may not be redefined with type "
                          ^ typeString tau)
                  end
          | ty (VALREC (name, tyex, exp)) =
              let  val new_env    = bind (name, tyex , type_env)
                   val texp       = typeof(exp, kind_env, new_env)
               in if eqType(texp, tyex) then (new_env, typeString tyex)
                  else raise TypeError
                                "expression type not the same as expected type"
               end
          | ty (EXP (exp)) =
                    let val tau = typeof (exp, kind_env, type_env)
                    in  (type_env, typeString tau)
                    end
          | ty (DEFINE (name, tyex, (vars, lambda_exp))) =
              if eqType(typeof(lambda_exp, kind_env, type_env), tyex) then
                  ty (VALREC (name, typeof (LAMBDA (vars, lambda_exp),
                              kind_env, type_env), LAMBDA (vars,lambda_exp)))
              else
                  raise TypeError "improper types"

              (*let val new_env = bind( name, tyex, type_env)
                  val vr_lambda = typeof(LAMBDA(lambda_exp), kind_env, new_env)
              in *)
    in
        ty d
    end
...
(*****************************************************************)
(*                                                               *)
(*   LEXICAL ANALYSIS AND PARSING FOR \TUSCHEME, PROVIDING [[FILEXDEFS]] AND
                                                             [[STRINGSXDEFS]] *)
(*                                                               *)
(*****************************************************************)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call
                                                                 [[ty]] S384b *)
...
(*****************************************************************)
(*                                                               *)
(*   IMPLEMENTATIONS OF \TUSCHEME\ PRIMITIVES AND DEFINITION OF
                                                             [[INITIALBASIS]] *)
(*                                                               *)
(*****************************************************************)
...
(* implementations of \tuscheme\ primitives and definition of [[initialBasis]]
   S394a *)
(* shared utility functions for building primitives in languages with type
   checking S389d *)
...
(*
          ...
          (":=", binaryOp(fn (ARRAY a, v) => (Array.update(a, 0, v); unitVal)
             | _ => raise BugInTypeChecking "improper parameters"),
             FORALL (["'a"], FUNTY ([reftype tvA, tvA], unittype))) ::

           ("ref", unaryOp(fn (s) => singletonArray (s)
              | _ => raise BugInTypeChecking "improper parameters"),
              FORALL (["'a"], FUNTY ([tvA], reftype tvA))) ::
              *)
...
(* if this space is completely empty, something goes wrong with the software O*)
                                                                             []
...
(* looks at command-line arguments and calls [[runAs]] to run the interpreter*)
